...
//private data
	...
	public:
		double eta, phi, nu, psi, tau, omega, step_size;
		double f;
		double *g, *revenue, *rev_bundle, *rev_sep;
		int num_boxes, prices_size, revenue_size;
		bool bundle;
	...
	for (int i = 0; i < g->num_boxes; i++){
		for (int j = 0; j < g->num_boxes; j++) {
			if ( check_for_bundle(p_b, i, j) && ( g->bundle == true ) ) {
				...
				else if ( bundle_check_p_2(p_t_1, i, j) ) {
					temp = g->buy_1(p_t_1, i, j);
					rev = temp;
				}	
				else {
					temp = g->buy_bundle(p_b, i, j);
				}
			}
