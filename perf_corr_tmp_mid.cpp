...
class DVD_Bundling
{
   ...
   #define WEEKS_TO_BUNDLING	104       //week before bundling occurs
   #define INFTY	10000
...
private:
	...
	double *rev_bundle; //portion of revenue for sales of bundle
	double *rev_sep; 	//portion revenue from sales of seperates
...
void DVD_Bundling::simulate_path(int time)
{
	...
	bundle = true ;		//bundle se caculation will now be activated
	p_t_1 = prices[t];
	//p_t_2 = INFTY; 
    	p_t_2 = prices[t - NEW_RELEASE_WEEK];
	a_t_1 = discount(t);
...
double DVD_Bundling::buy_bundle(double p_b, int i, int j){
	...
	g[GIDX(0,0)] += phi * (1-nu) * g[GIDX(i,j)];							//Already bought both
	g[GIDX(i,j)] = g_new;
...
int main()
{
    ...
    ofs.open("perf_corr_midpoint_revenues.csv");
    if (! ofs.is_open() ) {
