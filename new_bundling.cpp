...
//public methods
public:
    DVD_Bundling(double _eta, double _phi_1, double _phi_2, double _nu, double _psi, int _tau, double _omega, int _num_boxes, double *_prices, int _prices_size, double _rho, int _bundling_week, double _bundling_discount);
    ~DVD_Bundling();
    ...
    double buy_bundle_1_only(double p, int i, int j);
    double buy_bundle_2_only(double p, int i, int j);
...
/* constructor */
DVD_Bundling::DVD_Bundling(double _eta, double _phi_1, double _phi_2, double _nu, double _psi, int _tau, double _omega, int _num_boxes, double *_prices, int _prices_size, double _rho, int _bundling_week, double _bundling_discount)
{
   int i, j; 
   if (prn_log) 
...
double DVD_Bundling::buy_bundle(double p_b, int i, int j){
    double g_new;
    g[GIDX(0,0)] += phi_3 * g[GIDX(i,j)];                                                     //Already bought bot
    double r = p_b * phi_3 * g[GIDX(i,j)];

    if( (i == num_boxes -1) || (j == num_boxes -1) ) {g_new = 0;}
    else if ( i == j) {
        g_new = (1 - phi_3) * (1 - nu) * g[GIDX(i,j)] +  nu * (rho * eta * f_perf * step_size + (1-rho) * eta * f_ind * step_size * step_size);
    }
    else{
        g_new = (1 - phi_3) * (1 - nu) * g[GIDX(i,j)] + (1 - rho ) * nu * eta * f_ind * step_size * step_size ;
    }
...
double DVD_Bundling::buy_bundle_1_only(double p_t_1, int i, int j){
    double g_new;
    g[GIDX(0,j)] += phi_3 *  g[GIDX(i,j)];
    double r = p_t_1 * phi_3 * g[GIDX(i,j)];
...
double DVD_Bundling::buy_bundle_2_only(double p_t_2, int i, int j){
    double g_new;
    g[GIDX(0,j)] += phi_3 *  g[GIDX(i,j)];
    double r = p_t_2 * phi_3 * g[GIDX(i,j)];
...
double DVD_Bundling::buy_1(double p_t_1, int i, int j){
    double g_new;
    g[GIDX(0,j)] += phi_1 *  g[GIDX(i,j)];
    double r = p_t_1 * phi_1 * g[GIDX(i,j)];

    if( (i == num_boxes -1) || (j == num_boxes -1) ) {g_new = 0;}
    else if ( i == j ) {
        g_new =  (1 - phi_1) * (1 - nu) * g[GIDX(i,j)] +  nu * (rho * eta * f_perf * step_size + (1-rho) * eta * f_ind * step_size * step_size) ;
    }
    else{
        g_new = (1 - phi_1) * (1 - nu) * g[GIDX(i,j)] + (1 - rho) * nu * eta * f_ind * step_size * step_size ;
    }  
/*    g[GIDX(0,j)] += phi * (1 - nu) * g[GIDX(i,j)];                          //May buy DVD2  in future
    double r = p_t_1 * phi * g_new;  */
    g[GIDX(i,j)] = g_new;
...
double DVD_Bundling::buy_2 (double p_t_2,int i,int j){
    double g_new;
    g[GIDX(i,0)] += phi_2 * g[GIDX(i,j)];
    double r = p_t_2 * phi_2 * g[GIDX(i,j)];
    if( (i == num_boxes -1) || (j == num_boxes -1) ) {g_new = 0;}
    else if ( i == j ) {
        g_new = (1 - phi_2) *  (1 - nu) * g[GIDX(i,j)] +  nu * (rho * eta * f_perf * step_size + (1-rho) * eta * f_ind * step_size * step_size) ;
    }
    else {
        g_new = (1 - phi_2) * (1 - nu) * g[GIDX(i,j)] + (1 - rho) * nu * eta * f_ind * step_size * step_size ;
    }  
...
double DVD_Bundling::buy_both(double p_t_1, double p_t_2, int i, int j){
    double g_new;
    g[GIDX(0,0)] += phi_1 * g[GIDX(i,j)];
    g[GIDX(i,0)] += (phi_2 - phi_1) * g[GIDX(i,j)];
    double r = (p_t_1 + p_t_2) * phi_1 * g[GIDX(i,j)] + p_t_2 * (phi_2 - phi_1) * g[GIDX(i,j)];
    if( (i == num_boxes -1) || (j == num_boxes -1) ) {g_new = 0;}
    else if ( i == j ){
        g_new = (1 - phi_2) *(1 - nu) * g[GIDX(i,j)] +  nu * (rho * eta * f_perf * step_size + (1-rho) * eta * f_ind * step_size * step_size);
    }
        ...
        for (j = 0; j < num_boxes; j++){
            if ( (p_b <= (a_t_1*((double)i + 0.5) + a_t_2 *((double) j + 0.5))* step_size) && ( bundle == true) ){          //Buy bundle ?
                ...
                if ( (a_t_1 * ((double) i + 0.5) * step_size) < (p_t_1 - bundling_discount) ){
                    temp = buy_bundle_2_only(p_t_2, i, j);          
                    rev += temp;
                    rev_seperates += temp;
                }
                else if( (a_t_2 *((double) j + 0.5) * step_size) < (p_t_2 - bundling_discount) ){
                    temp = buy_bundle_1_only(p_t_1, i, j);
                    rev += temp;
                    rev_seperates += temp;
...
int main()
{
    ...
    DVD_Bundling test(2000, .024, .024, .005, 50, 10,  1.0, 1501, p_val, TSIZE, 0, 156,0 ); //test is instance of DVD_Bundling clas
   ...
   /* ofs <<" rho" << "," << "npv_bundle" <<","<<" npv_no_bundle"<<","<<"percent_gain" << endl;
	...
	double rho; 
    	for (int i = 0; i <= 20; i++  ){
			rho = 0.05 * i;
			cout<< rho << endl;
			DVD_Bundling test(2000, .024, 0.0010, .005, 50, 10,  1.0, 1501, p_val, TSIZE, rho , 208, 0);
			DVD_Bundling test2(2000, .024, .0010, .005, 50, 10,  1.0, 1501, p_val, TSIZE, rho , 260, 0);
	...
	for (int k = 100; k <= 200; k++){
		discount = k * 0.1;
		cout << discount << endl;
		DVD_Bundling test(2000, .024, .0010, .005, 50, 10,  1.0, 501, p_val, TSIZE, 1.0 , 156 , discount);
		test.simulate_path(TSIZE);
		test.NPV();
		npv = test.npv;
