...
//public methods
public:
    DVD_Bundling(double _eta, double _phi, double _nu, double _psi, int _tau, double _omega, int _num_boxes, double *_prices, int _prices_size, int _bundling_week);
    ~DVD_Bundling();
...
/* constructor */
DVD_Bundling::DVD_Bundling(double _eta, double _phi, double _nu, double _psi, int _tau, double _omega, int _num_boxes, double *_prices, int _prices_size, int _bundling_week)
{
   int i, j; 
   if (prn_log) 
...
int main()
{
    ...
    ofs.open("weekly_iteration_revenues_no_discount.csv");
    if (! ofs.is_open() ) {
	...
	ofs << "t" << "," << "npv" << "," << "npv_sep" << "," << "npv_bundle" << endl;
	//ofs << "t,npv,npv_sep,npv_bundle" << endl;

	for (int t = NEW_RELEASE_WEEK; t < TSIZE; t++){
		pTest = new DVD_Bundling(2000, .03, .005, 50, 3,  0.20, 1501, p_val, TSIZE, t);
		pTest->simulate_path(TSIZE);
		pTest->NPV();
		npv = pTest->npv;
...
/*
    DVD_Bundling test(2000, .03, .005, 50, 3,  0.20, 2541, p_val, TSIZE); //test is instance of DVD_Bundling clas
